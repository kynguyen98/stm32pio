...
def parse_args(args: list) -> Optional[argparse.Namespace]:
    ...
    for p in [parser_init, parser_new]:
        p.add_argument('-b', '--board', dest='board', help="PlatformIO name of the board")
    ...
    for p in [parser_new, parser_generate]:
        p.add_argument('--with-build', action='store_true', help="build a project after generation")
...
def main(sys_argv: Optional[list] = None) -> int:
    ...
    if args is not None and args.subcommand is not None and args.verbose:
        ...
        handler.setFormatter(stm32pio.util.DispatchingFormatter("%(levelname)-8s %(message)s", special=stm32pio.util.special_formatters))
    else:
    ...
    try:
        if args.subcommand == 'init':
            project = stm32pio.lib.Stm32pio(args.project_path, parameters={'project': {'board': args.board}}, instance_options={'save_on_destruction': True})
